...
export const useScheduleStore = defineStore('schedule', () => {
  ...
  const deleteSchedule = (contentId) => {
    ...
    if (schedule.isOfficial) {
      ...
      const modifiedIndex = modifiedOfficialSchedules.value.findIndex(
  ...
  // NEW: Move official schedule back to edit (make it unofficial)
  const moveScheduleBackToEdit = (contentId) => {
    const schedule = schedules.value.find((s) => s.contentId === contentId)
    ...
    if (!schedule.isOfficial) {
      console.warn('Schedule is already unofficial:', contentId)
      return false
    }

    // // Store original state for potential tracking
    // const originalDay = schedule.day
    // const originalOrder = schedule.order

    // Update the schedule to be unofficial
    schedule.isOfficial = false
    schedule.day = null
    schedule.order = null

    // Handle tracking:
    // 1. If this was a newly made official schedule, remove it from newOfficialSchedules
    const newOfficialIndex = newOfficialSchedules.value.findIndex((s) => s.contentId === contentId)
    if (newOfficialIndex !== -1) {
      newOfficialSchedules.value.splice(newOfficialIndex, 1)
      console.log('Removed from new official schedules tracking')
    } else {
      // 2. If this was an existing official schedule, add it to removedOfficialSchedules
      removedOfficialSchedules.value.push({ contentId })
      console.log('Added to removed official schedules tracking')
    }

    // 3. Remove from modified tracking if it exists
    const modifiedIndex = modifiedOfficialSchedules.value.findIndex(
      (s) => s.contentId === contentId,
    )
    if (modifiedIndex !== -1) {
      modifiedOfficialSchedules.value.splice(modifiedIndex, 1)
      console.log('Removed from modified official schedules tracking')
    }

    console.log(`Schedule "${schedule.name}" moved back to edit`)
  ...
  const trackOrderModification = (contentId, newDay, newOrder) => {
    ...
    if (isNewOfficial) {
      ...
      const newOfficialIndex = newOfficialSchedules.value.findIndex(
        (s) => s.contentId === contentId,
      )
      if (newOfficialIndex !== -1) {
    ...
    const existingIndex = modifiedOfficialSchedules.value.findIndex(
      (s) => s.contentId === contentId,
    )
    if (existingIndex !== -1) {
  ...
  return {
    ...
    moveScheduleBackToEdit, // NEW method
